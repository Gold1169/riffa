...
task TSK_TX_SYNCHRONIZE;
   ...
   begin
      ...
      if (tready_sw_ == `SYNC_CC_RDY) begin
         while (m_axis_rc_tready == 1'b0) begin
             @(posedge user_clk);
         end
      end else begin // tready_sw_ == `SYNC_RQ_RDY
         while (m_axis_cq_tready == 1'b0) begin
             @(posedge user_clk);
         end
      end
      //-----------------------------------------------------------------------\\
      if (active_ == 1'b1) begin
         ...
         TSK_READ_DATA_128(first_, last_call_,`TX_LOG,pcie_tlp_data,pcie_tlp_rem);
      end
      //-----------------------------------------------------------------------\\
      if (last_call_)
         TSK_PARSE_FRAME(`TX_LOG);
      //-----------------------------------------------------------------------\\
   end
